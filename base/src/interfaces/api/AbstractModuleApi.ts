...
export abstract class AbstractModuleApi<T extends IModule> implements IModuleApi {
  ...
  protected _addDataRoute(
    ...
    const _handler = async (
      ...
      try {
        return await Promise.resolve(method.call(this, request, reply));
      } catch (err: any) {
  ...
  protected registerSchema = (
    ...
    try {
      ...
      // Fastify/Ajv in this runtime does not include draft-2020-12 meta-schemas.
      // Remove $schema so body schemas generated by newer tools (e.g. Zod) can still register.
      if (
        typeof schemaCopy.$schema === 'string' &&
        schemaCopy.$schema.includes('json-schema.org/draft/2020-12')
      ) {
      ...
      const refPrefix = schemaIdPrefix ?? '';
      const rewriteDefinitionRefs = (value: unknown): void => {
        if (!value || typeof value !== 'object') {
          return;
        }
        if (Array.isArray(value)) {
          value.forEach((item) => rewriteDefinitionRefs(item));
          return;
        }
        ...
        if (typeof ref === 'string' && ref.startsWith('#/definitions/')) {
          record.$ref = ref.replace('#/definitions/', refPrefix);
        }

        Object.values(record).forEach((nested) => rewriteDefinitionRefs(nested));
      };

      rewriteDefinitionRefs(schemaCopy);
  ...
  protected _toMessagePath(input: CallAction, prefix?: string): string {
    ...
    const endpointAction = input.charAt(0).toLowerCase() + input.slice(1);
    return endpointPrefix.length > 0
      ? `/ocpp/${endpointVersion}/${endpointPrefix}/${endpointAction}`
      : `/ocpp/${endpointVersion}/${endpointAction}`;
  }
  ...
  protected _toDataPath(
    ...
    const endpointPrefix = (prefix || '').replace(/^\/+|\/+$/g, '');
    const endpointNamespace = input.charAt(0).toLowerCase() + input.slice(1);
    return endpointPrefix.length > 0
      ? `/data/${endpointPrefix}/${endpointNamespace}`
      : `/data/${endpointNamespace}`;
  }
}
